...
**Необязательные блоки:** если, например, нет сокетов, следующий за требованиями блок сразу содержит `Item Level:` — порядок секций в буфере не всегда совпадает с номерами блоков выше. Парсер дочитывает `Item Level:`, `Requires:`, `Sockets:` по всему тексту (по строкам), а не только по фиксированному индексу секции.
